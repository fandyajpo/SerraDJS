...
module.exports.run = async (client, message, args) => {
  ...
  // message.channel.send({ 'content' : `${guildLevel} cek`})
  ...
  if (message.member.voice.channel) {
    ...
    if (guildLevel === "NONE") {
      if (bitrate > 96)
        return message.reply(
          "<:False:823030995053576232> You cant set the bitrate higher than 96Kbps, because the server has lvl 0 boost"
        );
    }
    if (guildLevel === "TIER_1") {
      if (bitrate > 128)
        return message.reply(
          "<:False:823030995053576232> You cant set the bitrate higher than 128Kbps, because the server has lvl 1 boost"
        );
    }
    if (guildLevel === "TIER_2") {
      if (bitrate > 256)
        return message.reply(
          "<:False:823030995053576232> You cant set the bitrate higher than 256Kbps, because the server has lvl 2 boost"
        );
    }
    ...
    if (guildLevel === "TIER_3") {
      if (bitrate > 384)
        return message.reply(
          "<:False:823030995053576232> You cant set the bitrate higher than 384Kbps, because that was the mas bit"
        );
    }
  }
